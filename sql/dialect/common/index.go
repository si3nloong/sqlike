...
// HasIndex :
func (s *commonSQL) HasIndex(stmt db.Stmt, dbName, table string, idx sql.Index) {
...
// CreateIndexes :
func (s *commonSQL) CreateIndexes(stmt db.Stmt, db, table string, idxs []sql.Index, supportDesc bool) {
...
// DropIndexes :
func (s *commonSQL) DropIndexes(stmt db.Stmt, db, table string, idxs []string) {
	stmt.WriteString("ALTER TABLE " + s.TableName(db, table) + " ")
	for i, idx := range idxs {
		if idx == "PRIMARY" {
...
func (s *commonSQL) getIndexByType(k sql.Type) (idx string) {
	switch k {
	case sql.FullText:
		idx = "FULLTEXT INDEX"
	case sql.Spatial:
